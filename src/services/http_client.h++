...
namespace Ludwig {
  ...
  struct HttpClientRequest {
    ...
    HttpClientRequest(std::string url_str, std::string method) : url(*Url::parse(url_str).or_else([&url_str] -> std::optional<Url> {
      throw std::runtime_error(fmt::format("Invalid HTTP URL: {}", url_str));
    })), method(method) {
      if (!url.is_http_s()) {
        throw std::runtime_error(fmt::format("Not an HTTP(S) URL: {}", url_str));
      }
      fmt::format_to(
    ...
    inline auto redirect(std::string new_url) -> void {
      ...
      url = *Url::parse(new_url).or_else([&new_url] -> std::optional<Url> {
        throw std::runtime_error(fmt::format("Invalid HTTP URL: {}", new_url));
      });
      if (!url.is_http_s()) {
        throw std::runtime_error(fmt::format("Not an HTTP(S) URL: {}", new_url));
      }
  ...
  struct HttpClientRequestBuilder {
    ...
    auto dispatch()&& -> asio::awaitable<std::unique_ptr<const HttpClientResponse>>;
  };

  class HttpClient {
  protected:
    virtual auto fetch(std::unique_ptr<HttpClientRequest> req) -> asio::awaitable<std::unique_ptr<const HttpClientResponse>> = 0;
  public:
  ...
  inline auto HttpClientRequestBuilder::dispatch()&& -> asio::awaitable<std::unique_ptr<const HttpClientResponse>> {
