...
namespace Ludwig {
  auto make_jwt(const JwtPayload& payload, const uint8_t secret[JWT_SECRET_SIZE]) -> std::string {
    ...
    std::string buf;
    fmt::format_to(std::back_inserter(buf), "{}.{}", JWT_HEADER, Base64::encode(payload_os.str(), false));
    uint8_t sig[64];
    crypto_sha512_hmac(
  ...
  static inline auto parse_jwt_payload(const std::string_view& payload_b64) -> std::optional<JwtPayload> {
    JwtPayload payload;
  ...
  auto parse_jwt(std::string jwt, const uint8_t secret[JWT_SECRET_SIZE]) -> std::optional<JwtPayload> {
    ...
    const auto jwt_sv = std::string_view(jwt);
    ...
    if (
      ...
      jwt[header_len] != '.' ||
      jwt_sv.substr(0, header_len) != std::string_view(JWT_HEADER) ||
      // Find the '.' separating the payload and signature
      dot_ix == std::string::npos
    ) {
