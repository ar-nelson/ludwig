#include "lemmy_api_controller.h++"
...
namespace Ludwig::Lemmy {
  static inline auto listing_type_to_feed(ListingType lt, Ludwig::Login login) -> uint64_t {
    ...
    switch (lt) {
      case All: return PostController::FEED_ALL;
      case Local: return PostController::FEED_LOCAL;
      case Subscribed: return login ? PostController::FEED_HOME : PostController::FEED_LOCAL;
  ...
  auto ApiController::validate_jwt(ReadTxn& txn, SecretString&& jwt) -> uint64_t {
    if (auto parsed_jwt = parse_jwt(jwt.data, txn.get_jwt_secret())) {
      if (auto user_id = session_controller->validate_session(txn, parsed_jwt->sub)) return *user_id;
  ...
  auto ApiController::login_and_get_jwt(
    ...
    const auto session = session_controller->login(txn, username_or_email, std::move(password), ip, user_agent, true);
  ...
  // SiteView construction is split up to work around SiteController::update_site consuming its WriteTxn argument.

  static inline auto build_site_object(const SiteDetail* site) -> Site {
    const auto
  ...
  static inline auto build_site_aggregates(ReadTxn& txn) -> SiteAggregates {
  ...
  static inline auto build_site_view(
    const SiteDetail* site,
    const SiteAggregates& counts
  ) -> SiteView {
    const auto
      published = uint_to_timestamp(site->created_at),
      updated = uint_to_timestamp(site->updated_at);
    return {
      .site = build_site_object(site),
      .local_site = {
        .id = 0,
        .site_id = 0,
  ...
  auto ApiController::get_site_object() -> Site {
    return build_site_object(site_controller->site_detail());
  }

  auto ApiController::get_site_view(ReadTxn& txn) -> SiteView {
    return build_site_view(site_controller->site_detail(), build_site_aggregates(txn));
  }
  ...
  auto ApiController::change_password(WriteTxn& txn, ChangePassword& form, optional<SecretString>&& auth) -> LoginResponse {
    const auto [user_id, jwt] = require_auth_and_keep_jwt(form, std::move(auth), txn);
    if (form.new_password.data != form.new_password_verify.data) throw ApiError("Passwords do not match", 400);
    session_controller->change_password(txn, user_id, std::move(form.old_password), std::move(form.new_password));
    return { .jwt = SecretString(jwt.data), .registration_created = false, .verify_email_sent = false };
  }

  auto ApiController::create_comment(WriteTxn& txn, CreateComment& form, optional<SecretString>&& auth) -> CommentResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    const auto id = post_controller->create_local_comment(
      txn,
      user_id,
  ...
  auto ApiController::create_community(WriteTxn& txn, CreateCommunity& form, optional<SecretString>&& auth) -> CommunityResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    // TODO: use discussion_languages
    const auto id = board_controller->create_local_board(
      txn,
      user_id,
      form.name,
      form.title,
    ...
    if (form.icon || form.banner || form.description) {
      board_controller->update_local_board(txn, id, user_id, {
  ...
  auto ApiController::create_post(WriteTxn& txn, CreatePost& form, optional<SecretString>&& auth) -> PostResponse {
    if (form.honeypot && !form.honeypot->empty()) throw ApiError("bots begone", 418);
    const auto user_id = require_auth(form, std::move(auth), txn);
    // TODO: Use language_id
    const auto id = post_controller->create_local_thread(
      txn,
      user_id,
      form.community_id,
      form.name,
  ...
  auto ApiController::create_site(WriteTxn txn, CreateSite& form, optional<SecretString>&& auth) -> SiteResponse {
    require_auth(form, std::move(auth), txn, true);
    ...
    // TODO: federation
    // TODO: taglines
    const auto counts = build_site_aggregates(txn);
    first_run_controller->first_run_setup(std::move(txn), {{
      .name = form.name,
    ...
    return { .site_view = build_site_view(site_controller->site_detail(), counts), .taglines = {} };
  }

  auto ApiController::delete_account(WriteTxn& txn, DeleteAccount&, optional<SecretString>&&) -> void {
    throw ApiError("Not yet implemented", 500);
  }

  auto ApiController::delete_comment(WriteTxn& txn, DeleteComment&, optional<SecretString>&&) -> CommentResponse {
    throw ApiError("Not yet implemented", 500);
  }

  auto ApiController::delete_community(WriteTxn& txn, DeleteCommunity&, optional<SecretString>&&) -> CommunityResponse {
    throw ApiError("Not yet implemented", 500);
  }

  /* deleteCustomEmoji */

  auto ApiController::delete_post(WriteTxn& txn, DeletePost&, optional<SecretString>&&) -> PostResponse {
    throw ApiError("Not yet implemented", 500);
  }
  ...
  auto ApiController::edit_comment(WriteTxn& txn, EditComment& form, optional<SecretString>&& auth) -> CommentResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    // TODO: Use language_id
    post_controller->update_comment(txn, form.comment_id, user_id, {
  ...
  auto ApiController::edit_community(WriteTxn& txn, EditCommunity& form, optional<SecretString>&& auth) -> CommunityResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    // TODO: Use discussion_languages
    board_controller->update_local_board(txn, form.community_id, user_id, {
  ...
  auto ApiController::edit_post(WriteTxn& txn, EditPost& form, optional<SecretString>&& auth) -> PostResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    if (form.url) throw ApiError("Updating thread URLs is not yet implemented", 500);
    // TODO: Use language_id
    // TODO: Update url
    post_controller->update_thread(txn, form.post_id, user_id, {
  ...
  auto ApiController::edit_site(WriteTxn txn, EditSite& form, optional<SecretString>&& auth) -> SiteResponse {
    const auto user_id = require_auth(form, std::move(auth), txn, true);
    const auto home_page_type = form.default_post_listing_type
      .transform(parse_listing_type)
      .transform(listing_type_to_home_page_type);
    const auto registration_mode = form.registration_mode.transform(parse_registration_mode);
    ...
    // TODO: taglines
    const auto counts = build_site_aggregates(txn);
    site_controller->update_site(std::move(txn), {
      .name = form.name,
      .description = form.sidebar,
      .icon_url = form.icon,
      .banner_url = form.banner,
  ...
  auto ApiController::follow_community(WriteTxn& txn, FollowCommunity& form, optional<SecretString>&& auth) -> CommunityResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    board_controller->subscribe(txn, user_id, form.community_id, form.follow.value_or(true));
    return {
      .community_view = get_community_view(txn, form.community_id, user_id),
      .discussion_languages = vector<uint64_t>{1}
    };
  ...
  static inline auto paginate(const T& form, string_view endpoint) -> pair<uint16_t, uint16_t> {
    const uint16_t limit = form.limit ? form.limit : 20, page = form.page ? form.page - 1 : 0;
  ...
  static inline auto page_to_vector(uint16_t offset, Fn fn, std::generator<const In&> gen, uint16_t limit) -> vector<decltype(fn(std::declval<const In&>()))> {
    vector<decltype(fn(std::declval<const In&>()))> out;
    uint32_t i = 0, max = offset + limit;
    for (const In& e : gen) {
  ...
  auto ApiController::get_comment(ReadTxn& txn, const GetComment& form, optional<SecretString>&& auth) -> CommentResponse {
  ...
  auto ApiController::get_comments(ReadTxn& txn, const GetComments& form, optional<SecretString>&& auth) -> GetCommentsResponse {
    ...
    if (form.parent_id || form.post_id) {
      ...
      CommentTree tree;
      if (is_thread) {
        post_controller->thread_detail(txn, tree, parent_id, sort, login, next, limit);
      } else {
        entries.push_back(to_comment_view(txn,
          post_controller->comment_detail(txn, tree, parent_id, sort, login, next, limit)));
      }
      ...
      if (!form.community_name.empty()) {
        if (auto board_id = txn.get_board_id_by_name(form.community_name)) {
          entries = page_to_vector(offset, [&](auto& e) { return to_comment_view(txn, e); },
           post_controller->list_board_comments(txn, next, *board_id, sort, login), limit);
        } else {
          throw ApiError(fmt::format("No community named \"{}\" exists", form.community_name), 410);
        }
      } else {
        const auto feed = form.type ? listing_type_to_feed(*form.type, login) : PostController::FEED_ALL;
        entries = page_to_vector(offset, [&](auto& e) { return to_comment_view(txn, e); },
          post_controller->list_feed_comments(txn, next, feed, sort, login), limit);
      }
    }
    return { entries };
  }

  auto ApiController::get_community(ReadTxn& txn, const GetCommunity& form, optional<SecretString>&& auth) -> GetCommunityResponse {
    const auto login_id = auth.transform([&](auto&& s){return validate_jwt(txn, std::move(s));});
  ...
  auto ApiController::get_person_details(ReadTxn& txn, const GetPersonDetails& form, optional<SecretString>&& auth) -> GetPersonDetailsResponse {
    ...
    return {
      ...
      .comments = page_to_vector(offset, [&](auto& e) { return to_comment_view(txn, e); },
        post_controller->list_user_comments(txn, c1, id, form.sort, login), limit),
      .posts = page_to_vector(offset, [&](auto& e) { return to_post_view(txn, e); },
        post_controller->list_user_threads(txn, c2, id, form.sort, login), limit)
      // TODO: moderators
      // TODO: iterate posts and comments separately?
    };
  }

  auto ApiController::get_person_mentions(ReadTxn& txn, const GetPersonMentions&, SecretString&& auth) -> GetPersonMentionsResponse {
    validate_jwt(txn, std::move(auth));
    // TODO: mentions
    return {};
  }

  auto ApiController::get_post(ReadTxn& txn, const GetPost& form, optional<SecretString>&& auth) -> GetPostResponse {
  ...
  auto ApiController::get_posts(ReadTxn& txn, const GetPosts& form, optional<SecretString>&& auth) -> GetPostsResponse {
    ...
    if (board_id) {
      return { page_to_vector(offset, [&](auto& e) { return to_post_view(txn, e); },
        post_controller->list_board_threads(txn, cur, board_id, sort, login), limit) };
    }
    auto feed = form.type ? listing_type_to_feed(*form.type, login) : PostController::FEED_ALL;
    return { page_to_vector(offset, [&](auto& e) { return to_post_view(txn, e); },
      post_controller->list_feed_threads(txn, cur, feed, sort, login), limit) };
  }

  /* getPrivateMessages */

  auto ApiController::get_replies(ReadTxn& txn, const GetReplies&, SecretString&& auth) -> GetRepliesResponse {
    validate_jwt(txn, std::move(auth));
  ...
  auto ApiController::get_site(ReadTxn& txn, optional<SecretString>&& auth) -> GetSiteResponse {
    const auto login_id = auth.transform([&](auto&& s){return validate_jwt(txn, std::move(s));});
    const auto login = LocalUserDetail::get_login(txn, login_id);
  ...
  auto ApiController::like_comment(WriteTxn& txn, CreateCommentLike& form, optional<SecretString>&& auth) -> CommentResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    if (form.score > 1 || form.score < -1) throw ApiError("Invalid vote score (must be -1, 0, or 1)", 400);
    post_controller->vote(txn, user_id, form.comment_id, (Vote)form.score);
    return { get_comment_view(txn, form.comment_id, user_id), {}, {} };
  }

  auto ApiController::like_post(WriteTxn& txn, CreatePostLike& form, optional<SecretString>&& auth) -> PostResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    if (form.score > 1 || form.score < -1) throw ApiError("Invalid vote score (must be -1, 0, or 1)", 400);
    post_controller->vote(txn, user_id, form.post_id, (Vote)form.score);
    return { get_post_view(txn, form.post_id, user_id) };
  }

  /* listCommentReports */

  auto ApiController::list_communities(ReadTxn& txn, const ListCommunities& form, optional<SecretString>&& auth) -> ListCommunitiesResponse {
    auto [offset, limit] = paginate(form, "list_communities");
    const auto login_id = auth.transform([&](auto&& s){return validate_jwt(txn, std::move(s));});
    const auto login = LocalUserDetail::get_login(txn, login_id);
    PageCursor cur;
    // TODO: hide nsfw
    return { page_to_vector(offset, [&](auto& e) { return to_community_view(e); }, board_controller->list_boards(
      txn, cur, form.sort,
  ...
  auto ApiController::login(WriteTxn& txn, Login& form, string_view ip, string_view user_agent) -> LoginResponse {
    // TODO: Specific error messages from API
    if (!form.totp_2fa_token.value_or("").empty()) {
      throw ApiError("TOTP 2FA is not supported", 400);
    }
    return {
      login_and_get_jwt(txn, form.username_or_email, std::move(form.password), ip, user_agent),
      false, false
    };
  }
  ...
  auto ApiController::logout(WriteTxn& txn, SecretString&& auth) -> void {
    ...
    if (session_id) session_controller->delete_session(txn, session_id);
  }

  auto ApiController::mark_all_as_read(WriteTxn& txn, MarkAllAsRead& form, optional<SecretString>&& auth) -> GetRepliesResponse {
    require_auth(form, std::move(auth), txn);
    // TODO: Support mark as read (this does nothing)
    return {};
  }

  auto ApiController::mark_comment_reply_as_read(WriteTxn& txn, MarkCommentReplyAsRead& form, optional<SecretString>&& auth) -> CommentReplyResponse {
    require_auth(form, std::move(auth), txn);
    // TODO: Support mark as read (this does nothing)
    return {};
  }

  auto ApiController::mark_person_mentions_as_read(WriteTxn& txn, MarkPersonMentionAsRead& form, optional<SecretString>&& auth) -> PersonMentionResponse {
    require_auth(form, std::move(auth), txn);
    // TODO: Support mark as read (this does nothing)
    return {};
  }

  auto ApiController::mark_post_as_read(WriteTxn& txn, MarkPostAsRead& form, optional<SecretString>&& auth) -> PostResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    // TODO: Support mark as read (this does nothing)
  ...
  auto ApiController::password_change_after_reset(WriteTxn& txn, PasswordChangeAfterReset& form) -> void {
    if (form.password.data != form.password_verify.data) throw ApiError("Passwords do not match", 400);
    session_controller->change_password(txn, form.token, std::move(form.password));
  }
  ...
  auto ApiController::register_account(WriteTxn& txn, Register& form, string_view ip, string_view user_agent) -> pair<uint64_t, bool> {
    if (form.honeypot && !form.honeypot->empty()) throw ApiError("bots begone", 418);
    if (form.password.data != form.password_verify.data) throw ApiError("Passwords do not match", 400);
    // TODO: Use captcha
    // TODO: Use show_nsfw
    return session_controller->register_local_user(
      txn,
      form.username,
      form.email,
  ...
  auto ApiController::save_comment(WriteTxn& txn, SaveComment& form, optional<SecretString>&& auth) -> CommentResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    user_controller->save_post(txn, user_id, form.comment_id, form.save.value_or(true));
    return { get_comment_view(txn, form.comment_id, user_id), {}, {} };
  }

  auto ApiController::save_post(WriteTxn& txn, SavePost& form, optional<SecretString>&& auth) -> PostResponse {
    const auto user_id = require_auth(form, std::move(auth), txn);
    user_controller->save_post(txn, user_id, form.post_id, form.save.value_or(true));
    return { get_post_view(txn, form.post_id, user_id) };
  }

  auto ApiController::save_user_settings(WriteTxn& txn, SaveUserSettings& form, optional<SecretString>&& auth) -> LoginResponse {
    const auto [user_id, jwt] = require_auth_and_keep_jwt(form, std::move(auth), txn);
    ...
    // TODO: theme
    user_controller->update_local_user(txn, user_id, user_id, {
      .email = form.email,
  ...
  auto ApiController::search_results(ReadTxn& txn, const vector<SearchResultDetail>& results) -> SearchResponse {
  ...
  auto ApiController::validate_auth(ReadTxn& txn, std::optional<SecretString>&& auth) -> void {
    if (auth) validate_jwt(txn, std::move(*auth));
  }
