...
void poll_for_error(struct boost_block_poll_t *boost_block) {
    ...
    polls++;
    boost_block->async_wait(asio::posix::descriptor::wait_type::wait_error, [nr = boost_block->nr, weakBoostBlock = std::weak_ptr<boost_block_poll_t>(boost_block->isValid)](asio::error_code ec) {
        polls--;
...
inline void handle_read(const std::weak_ptr<boost_block_poll_t> &weakBoostBlock, unsigned char nr, asio::error_code ec) {
    if (ec != asio::error::operation_aborted) {
...
void poll_for_read(struct boost_block_poll_t *boost_block) {
    polls++;
#ifndef LIBUS_USE_OLD_ASIO
    boost_block->async_wait(asio::posix::descriptor::wait_type::wait_read, [nr = boost_block->nr, weakBoostBlock = std::weak_ptr<boost_block_poll_t>(boost_block->isValid)](asio::error_code ec) {
        polls--;
        handle_read(weakBoostBlock, nr, ec);
    });
#else
    boost_block->async_read_some(asio::null_buffers(), [nr = boost_block->nr, weakBoostBlock = std::weak_ptr<boost_block_poll_t>(boost_block->isValid)](asio::error_code ec, std::size_t) {
        polls--;
        handle_read(weakBoostBlock, nr, ec);
    });
#endif
...
inline void handle_write(const std::weak_ptr<boost_block_poll_t> &weakBoostBlock, unsigned char nr, asio::error_code ec) {
    if (ec != asio::error::operation_aborted) {
...
void poll_for_write(struct boost_block_poll_t *boost_block) {
    polls++;
#ifndef LIBUS_USE_OLD_ASIO
    boost_block->async_wait(asio::posix::descriptor::wait_type::wait_write, [nr = boost_block->nr, weakBoostBlock = std::weak_ptr<boost_block_poll_t>(boost_block->isValid)](asio::error_code ec) {
        polls--;
        handle_write(weakBoostBlock, nr, ec);
    });
#else
    boost_block->async_write_some(asio::null_buffers(), [nr = boost_block->nr, weakBoostBlock = std::weak_ptr<boost_block_poll_t>(boost_block->isValid)](asio::error_code ec, std::size_t) {
        polls--;
        handle_write(weakBoostBlock, nr, ec);
    });
#endif
...
void poll_for_timeout(struct boost_timer *b_timer, int repeat_ms) {
    b_timer->timer.async_wait([nr = b_timer->nr, repeat_ms, weakBoostBlock = std::weak_ptr<boost_timer>(b_timer->isValid)](const asio::error_code &ec) {
        if (ec != asio::error::operation_aborted) {
